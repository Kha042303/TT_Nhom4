...
export default function TTSeller({ seller }: { seller?: SellerInfo }) {
  return (
    <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
      <div className="flex items-center justify-between">
        <div className="text-sm font-extrabold text-slate-900">THÔNG TIN NGƯỜI BÁN</div>
        <div className="flex items-center gap-2">
          <span className={["h-2 w-2 rounded-full", seller?.online ? "bg-emerald-500" : "bg-slate-300"].join(" ")} />
      ...
      <div className="mt-4 flex items-center gap-3">
        ...
        <div className="min-w-0">
          <div className="font-extrabold text-slate-900">
            {seller?.name ?? <span className="inline-block h-5 w-40 bg-slate-100 animate-pulse rounded-md" />}
          </div>
      ...
      <button
        type="button"
        className="mt-5 w-full rounded-xl bg-sky-600 px-4 py-3 font-extrabold text-white shadow hover:bg-sky-700 inline-flex items-center justify-center gap-2"
      >
